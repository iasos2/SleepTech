...
public class E_Map {
	public static void main(String[] args) {
		...
		// {조민지=24, 이승아=29, 이도경=31}
		// : {키=값, 키=값} - 키와 값의 구분을 = 로 표시
		// 삽입 순서 보장 안됨
		
		// 2. get(키): 지정된 키의 값을 반환
		System.out.println(students.get("조민지")); // 24
		
		// 3. remove(키) : 지정된 키의 값을 삭제 후 반환
		students.remove("조민지");
		System.out.println(students); // {이승아=29, 이도경=31}
		
		// 4. containsKey(키): 해당 키가 Map에 있는지 확인하여 boolean 값을 반환
		System.out.println(students.containsKey("조민지")); // false
		System.out.println(students.containsKey("이승아")); // true
		
		// 5. size()
		System.out.println(students.size()); // 2
		
		// 6. keySet(): Map의 키를 Set으로 반환
		System.out.println(students.keySet()); // [이승아, 이도경]
		
		// 7. values(): Map의 값을 컬렉션타입으로 반환
		Collection<Integer> ages = students.values(); 
		System.out.println(ages); // [29, 31]
		
		// cf) Collection 타입은 하위 인터페이스로 반환 가능
		List<Integer> listAges = new ArrayList<>(ages);
		System.out.println(listAges); // [29, 31]
		
		// 8. entrySet(): Map의 키와 값을 Set으로 반환
		System.out.println(students.entrySet()); // [이승아=29, 이도경=31]
	}
}
